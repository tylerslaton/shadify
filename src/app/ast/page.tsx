    ...
    >
      ...
      {layout.nodes.map((node) => {
        ...
        return (
          <div
            key={node.id}
            className={`absolute rounded-2xl border border-[rgba(164,163,161,0.45)] bg-white/88 p-3 shadow-lg backdrop-blur ${
          ...
          >
            <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-[var(--gray)]">
              <span>{node.type}</span>
              <span className="text-[9px] text-[var(--gray-light)]">
                {node.closed ? "closed" : "open"}
              </span>
            </div>
            <div
              className={`mt-2 truncate text-sm font-semibold text-[var(--foreground)] ${
            ...
            <div className="mt-1 text-[10px] text-[var(--gray-light)]">
              id {node.id}
            </div>
          </div>
        );
    ...
    <main className="flex h-dvh w-full flex-col bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[minmax(0,1fr)_720px]">
        <section className="flex min-h-0 flex-col border-b border-[rgba(164,163,161,0.45)] bg-white/55 md:border-b-0 md:border-r">
          ...
          <div className="flex min-h-0 flex-[0.6] flex-col border-t border-[rgba(164,163,161,0.45)]">
            <div className="border-b border-[rgba(164,163,161,0.45)] px-6 py-3 text-xs uppercase tracking-[0.3em] text-[var(--gray)]">
              Resolved Value
            </div>
            <div className="min-h-0 flex-1 overflow-auto px-6 py-4">
          ...
          {ticks.map((_, index) => (
            <span
              ...
              className={`h-full rounded-sm ${
                index < cursor ? "bg-[var(--sunset-orange)]" : "bg-[var(--gray-light)]/60"
              }`}
            />
          ))}
        </div>
